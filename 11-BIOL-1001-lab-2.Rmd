...
This lab is worth 5% of your final mark. Before completing this lab, you should re-familiarize yourself with Chapters \@ref(intro), \@ref(data), \@ref(hypotheses) and \@ref(graphs). If you need to consult the help files, use Chapter \@(help) to help guide you through the ```R``` help.
...
1. We provide this data as a .csv called `Langdon_et-al_2000_dataset.csv`. Follow the instructions in Chapter \@ref(data-entry) and import the data. Note that this is just a subset of the data from the full paper. 
...
1. Follow the instructions in Chapter \@ref(graph) to make a scatterplot for these data and replicate the figure from the Campbell textbook (see Figure A.1 below)
...
**NEW TIP** instead of entering the data as a vector the way you learned in Chapter \@ref(graph), you can use the following code to create an x vector from a data frame (here I titled my data frame "langdon"; your name may be different, and created a vector of data called "CO3"). You can use this to create the vector of data that you need to plot on the x-axis, and then write you own code to create the vector of data for the y-axis. 

```{r}
langdon.data <- read.csv("Langdon_et-al_2000.csv")
CO3 <- langdon.data$CO3
```

Use the examples in Chapter \@ref(graph) to create a fully labelled scatter plot. Export this and paste it into your document that you will hand in for this lab. 

**HINT** if you want to add a line of best fit based on linear regression, use the code snippet below. The two terms on each side of the tilde ("~" symbol) tell R what the equation for the regression ```y~x``` is. Your terms for ```x``` and ```y``` might be different. You can chose a different colour (```col```) or line width (```lwd```).

```{r}
abline(lm(calcification~CO3), col = "red", lwd = 2)
```

```{r panes, fig.cap="Figure A.1. Scatterplot from Langdon et al. study of the effect of carbon dioxide on ocean acification. This figure is found in your textbook on pg. 60", fig.align='center', out.width="70%", fig.show='hold', echo=FALSE}
knitr::include_graphics('./figures/Langdon_scatterplot.jpeg')
```

1. Follow the instructions in Chapter \@ref(graph) to make a line graph for these data. Export your line graph insert it into your lab report. Write a figure caption. 
...
### Questions {-}
1. Follow the instructions in Chapter \@ref(data-entry) to load the data file Falie_et-al_2011_dataset.csv using the command line (```read.csv```) option. **Note** that the data in the CSV file are ordered differently than in the list above (to verify this you can open the CSV file in Excel to see the full set).
...
1. Follow the instructions below to replicate the figure from the Campbell textbook (bar plot; see Figure A.2 below). Export your bar graph insert it into your lab report. Add a caption to your bar plot and hand it in with your lab report.

```{r panes, fig.cap="Figure A.2. Barplot from Falik et al. study on plant stress and plant communication. This figure is found in your textbook on pg. 910", fig.align='center', out.width="70%", fig.show='hold', echo=FALSE}
knitr::include_graphics('./figures/Falik_barplot.jpeg')
```


To make a barplot, use the code below as a starting point, and then use the resources in this guide to add appropriate x and y axis labels.

```{r}
width <- falik.data$width #This creates a vector of data of the plant stomatal widths
plant <- falik.data$plant #The creates a vector of data of the plant ID numbers
...
1. Follow the instructions below to re-plot the data from EXERCISE 1 as a boxplot that compares control versus treatment. Export your boxplot and insert it into your lab report. Add a caption to your boxplot.

````{r}
control <- subset(falik.data, type == "control") #This subset command is a handy function to break the data into subsets. Here, we're creating a data frame with just the values from the control.
treatment <- subset(falik.data, type == "15min") #Here we are subsetting the data to contain only the data from the treatment

boxplot(control$width, treatment$width, names = c("contro", "treatment"), ylab = "stomatal opening (microns)")

```

1.	Label the following on the boxplot: mean, 25% and 75% quantiles, 95% confidence limits, outliers. **HINT** you can insert the exported graph into a blank PowerPoint slide and use lines/text boxes to draw the labels. Then save the slide as an image file (e.g., JPEG) and insert it in your lab report.
