...
The proposed answer to a problem is termed a **hypothesis**. Hypotheses are formulated on the basis of some preliminary observations in combination with prior knowledge.
	
How does the presence of a concentration gradient affect the direction of osmosis?  There are three possible answers to this problem.
...
If hypothesis #2 holds true water will diffuse from the hypotonic solution (the water in the beaker) into the hypertonic solution (the 30% glucose and dye solution in the osmometer) and the water will rise in the pipette. Note how this prediction was generated using the if ... then ... format.
...
### Null hypotheses
Null means nothing or zero and null hypotheses usually describe outcomes where the independent variables have no effect. Null hypotheses are usually paired with alternative hypotheses whereby the independent variable does have an affect. As such, H3 is a potential null hypothesis, since this hypothesis describes the concentration gradient having no effect on the direction of difussion, and could be paired with the alternative hypothesis, H2. Null hypothesis testing compares the data to the null hypothesis, and assesses the probability that such data could be recorded if the null hypothesis were true. When written as an equation, frequently null hypotheses are expressed with as a given parameter set equal to zero.
...
Historically, many statistical analyses have been performed in the null hypothesis testing framework. More recently, information theoretic approaches that assess the relative support for each of multiple working hypotheses have gained popularity (burnham and anderson).
...
Consider an expanded version of the previous experiment, where we now consider concentrations of 30, 60, and 90% glucose in the osmometer, all in beakers of distilled water.
...
<strong>Null Hypothesis:</strong> The hypertonicity of the solution has no effect of diffusion.

<strong>Equation under the null hypothesis:</strong> The null hypothesis is represented by setting $b=0$ in the above equation.
...
<strong>Null Hypothesis:</strong> Concentration gradients do not affect difussion.

<strong>Equation under the null hypothesis:</strong> At least two of mean volumes of liquid in the osmometer at the end of the experiment are equal, i.e., at least one of the following is true:
