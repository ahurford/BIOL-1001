...
The assignment operator is `<-`. You create this by typing the "less than" sign (the `<` symbol located above the comma) followed by a hyphen. It stores whatever you write on the right to variables on
the left. Variable names can be made up of letters, numbers, underscores and periods. However, they cannot start with a number nor contain spaces. For instance:
...
but if you look at the `Environment` panel, you will see the new variable there. (REMEMBER: if you just type the above in the script without sourcing it, your value for `x` will not necessarily match. Either source the script or type the above in the console).

You can also check that it is stored by calling the variable, simply by typing it in the console:
...
In R, the `#` sign is a special character that indicates a comment. Anything that is written directly to the right of a `#` sign will be ignored by R. Comment your scripts often! You can use a whole line of `####` symbols to create blocks as headings to make your script easier to read.
