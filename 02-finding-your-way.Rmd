...
This is where you create and edit `R` scripts. `R` scripts are lists of commands which have meanings due to functions defined in `R` and when saved have a `.R` extension. When you've written and saved a script, you can re-use it for future projects. Try it now, open and save a new `.R` file to save your work in. In the top menu select `File > New File > R Script`, then `File > Save As`. 

Code that is written in the `Source` pane is generally read from top to bottom when you click the `Source` button, which appears on the righthand side of the `Source` pane if your pane is sufficiently large. If you have a small pane and this button does not appear, you can equivalently select `Code > Source` from the top menu bar.
