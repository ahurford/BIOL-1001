...
Elements of these data types may be combined to form data structures, such as vectors.
A *vector* is a set of values, in a specific order, and of the same data type. 
Further, vectors are *atomic*, which means that they can only contain one class of data.
For example:
...
**TRY IT!** What are the data types and lengths of the vectors `rgb` and `y`?
...
A key point of the previous example is that vectors can be used as arguments to functions. But, different classes cannot be mixed in the same object. For instance, if we combine numeric with character data types, R will coerce all elements to be characters:
